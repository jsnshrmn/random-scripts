...
if ($ARGV[0]) {
  ...
  (POISE) and spits out a CSV that we can bring into Koha.

  Accepts the absolute filename of the POISE CSV optionally followed by category code and branch code.
  Then press enter.  Examples:
  "C:\Users\You\Downloads\POISE.CSV"
  Imports all users as students with Nash Library as their home branch.
  
  "C:\Users\You\Downloads\POISE.CSV" AL DIGITAL
  Imports all users as alumni with Nash Digital as their home branch.
  
  In Koha,  see Administration > Libraries and groups
            and Administration > Patron categories
  for more information.
  
  Enter the absolute filename of the POISE CSV:
  ';
  $file = <STDIN>;
  chomp $file;
  if ($file =~ m/^("(.+)"|[^\s]+)\s?(\w+)?\s?(\w+)?$/) {
...
my $outputfile = File::Spec->catfile($filedir,"KohaUserImport.csv");

##print header to file
open(MYOUTPUTFILE, ">", "$outputfile") || die("Could not open $outputfile!"); 
print MYOUTPUTFILE "cardnumber\,surname\,firstname\,dateofbirth\,branchcode\,categorycode\,address\,address2\,city\,zipcode\,country\,email\,emailpro\,phone\,phonepro\,B_address\,B_address2\,B_city\,B_zipcode\,B_country\,password\,userid\n";
close(MYOUTPUTFILE);
...
while (my $row = $csv->getline_hr ($io)) {
  if ($csv->parse($row)) {
    ...
    if ($row->{cardnumber}) {
      ...
      #Print data to file
      open(MYOUTPUTFILE, ">>", "$outputfile") || die("Could not open $outputfile!");
      print MYOUTPUTFILE "\"$row->{cardnumber}\"\,\"$row->{surname}\"\,\"$firstname\"\,\"$row->{dateofbirth}\"\,\"$branchcode\"\,\"$categorycode\"\,\"$address\"\,\"$address2\"\,\"$city\"\,\"$zipcode\"\,\"$country\"\,\"$email\"\,\"$emailpro\"\,\"$phone\"\,\"$phonepro\"\,\"$B_address\"\,\"$B_address2\"\,\"$B_city\"\,\"$B_zipcode\"\,\"$B_country\"\,\"$row->{password}\"\,\"$userid\"\n";
      close(MYOUTPUTFILE);
    }
  } else {
